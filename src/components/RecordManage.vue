  ...
  <div class="container">
    <Form ref="formInline" :model="formInline" :rules="ruleInline" inline>
      <FormItem prop="borrowRecord">
        <Input type="text" v-model="formInline.raccount" placeholder="借阅者账号">
  ...
  export default {
    ...
    methods: {
      ...
      show (index) {
        this.$Modal.info({
          title: '期刊信息',
          content: `期刊名：${this.data6[index].title}<br>期刊编号：${this.data6[index].number}<br>借阅者账号：${this.data6[index].raccount}<br>借阅者姓名：${this.data6[index].rname}<br>借阅时间：${this.data6[index].time}<br>应归还时间：${this.data6[index].backtime}<br>状态：${this.data6[index].condi}`
        })
      },
      reback (index) {
